...
class resizer final {
public:
	/// Horizontal and vertical behavior of a control when the parent window is resized.
	enum class go {
		/// Control width/height is fixed; control moves around anchored.
		REPOS,
		/// Control width/height stretches; control doesn't move.
		RESIZE,
		/// Fixed width/height; control doesn't move.
		NOTHING
	};
...
public:
	...
	/// Adds new child controls to be resized when parent resizes.
	...
	/// @see https://docs.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-begindeferwindowpos
	void adjust(msg::wm_size p) noexcept
	{
		...
		for (const ctrl& control : this->_ctrls) {
			...
			DeferWindowPos(hdwp, control.hChild, nullptr,
				...
				p.client_area_size().cx - this->_szOrig.cx + control.rcOrig.left :
				control.rcOrig.left, // keep original pos
				control.modeVert == go::REPOS ?
				p.client_area_size().cy - this->_szOrig.cy + control.rcOrig.top :
				control.rcOrig.top, // keep original pos
				control.modeHorz == go::RESIZE ?
				p.client_area_size().cx - this->_szOrig.cx + control.rcOrig.right - control.rcOrig.left :
				control.rcOrig.right - control.rcOrig.left, // keep original width
				control.modeVert == go::RESIZE ?
				p.client_area_size().cy - this->_szOrig.cy + control.rcOrig.bottom - control.rcOrig.top :
